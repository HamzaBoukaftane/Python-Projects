...
def creerMolecule(x, y, dx, dy, rayon):
    ...
    mol= {"x": x,"y": y , "dx": dx, "dy": dy, "rayon": rayon}
    return mol
...
def moleculesSeTouche(mol_1, mol_2):
    ...
    def distanceMolecule(mol_1,mol_2):
        differenceH= mol_1["x"] - mol_2["x"]
        differenceV= mol_1["y"] - mol_2["y"]
        d = (differenceH ** 2 + differenceV ** 2) ** (1/2)

        return d

    def sommeRayon(mol_1,mol_2):
        sommeR= mol_1["rayon"]+mol_2["rayon"]
        return sommeR

    result=True

    if distanceMolecule(mol_1,mol_2) >= sommeRayon(mol_1,mol_2):
        result= False
...
def creerListMolecules(hauteur,xmin,xmax,nbMolecules):
    ...
    for molecule in range(nbMolecules):
        ...
        mol = creerMolecule(randint(xmin + rayon, xmax - rayon),
                            randint(rayon, hauteur - rayon),
                            randint(15, 25),
                            randint(15, 25),
                            rayon)
        listMol.append(mol)
...
def inverseDirMolecule(mol, paroiG, paroiD, hauteur):
    ...
    # Si la molécule touche la paroi inférieure du réservoir en y
